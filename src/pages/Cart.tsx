          ...
          {items.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white p-6 rounded-lg shadow h-fit">
                ...
                <div className="mt-6">
                  <Button onClick={handleCheckout} disabled={isLoading || hasUnavailableItems} className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-gray-400">
                    {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                    Proceed to checkout
                  </Button>
                </div>
                <div className="mt-4 flex justify-center text-center text-sm text-gray-500">
                  <p>or <Link to="/products" className="font-medium text-orange-600 hover:text-orange-500">Continue Shopping<span aria-hidden="true"> &rarr;</span></Link></p>
                </div>
              </div>
            </div>
          ) : (
